const POINTS=[
 ...
 {code:"",name:"Schleuse Brunsbüttel",km:1.6}
];
...
function load(){try{const x=JSON.parse(localStorage.getItem("nokNavigatorV1")||"null");if(x){
 const restore=saved=>POINTS.map((_,i)=>{const v=Array.isArray(saved)?saved[i]:null;return v&&v.fixed&&v.min!=null?{min:v.min,fixed:true}:{min:null,fixed:false}});
 east=restore(x.east);west=restore(x.west);$("shipEast").value=x.shipEast||"";$("shipWest").value=x.shipWest||"";$("speedEast").value=x.speedEast||"6.5";$("speedWest").value=x.speedWest||"6.5";$("rueLimit").value=x.rueLimit||"15";$("showRue").checked=x.showRue!==false;$("showDu").checked=x.showDu!==false;$("showGn").checked=x.showGn!==false}}catch(e){console.warn(e)}}
$("speedEast").addEventListener("change",()=>recalc("E"));$("speedWest").addEventListener("change",()=>recalc("W"));$("rueLimit").addEventListener("input",()=>{render();save()});["showRue","showDu","showGn"].forEach(id=>$(id).addEventListener("change",()=>{render();save()}));["shipEast","shipWest"].forEach(id=>$(id).addEventListener("input",()=>{$(id).value=$(id).value.toUpperCase();render();save()}));$("clearEast").addEventListener("click",()=>clearDir("E"));$("clearWest").addEventListener("click",()=>clearDir("W"));$("unlockAll").addEventListener("click",()=>{east.forEach(x=>x.fixed=false);west.forEach(x=>x.fixed=false);render();save()});$("resetAll").addEventListener("click",()=>{localStorage.removeItem("nokNavigatorV1");location.reload()});load();if(east.some(x=>x.fixed&&x.min!=null))recalc("E");if(west.some(x=>x.fixed&&x.min!=null))recalc("W");render();save();
